...
header_match = re.search(r'(.*?<!-- Main Content -->\s*<main id="mainContent">\s*)', content, re.DOTALL)
header = header_match.group(1)

footer_match = re.search(r'(\s*</main>\s*<!-- Footer -->.*)', content, re.DOTALL)
footer = footer_match.group(1)

# Helper to create a single page
def create_page(filename, title, banner_title, banner_desc, hero_img, page_content):
    # Modify header title
    page_header = re.sub(r'<title>.*?</title>', f'<title>{title} | D & A Holdings Health Solutions LLC</title>', header)
    
    # Modify hero section
    hero_section = f"""    <!-- Panoramic Banner Hero with Semi-Transparent Blue Overlay -->
    <section class="banner-hero-section">
        <img src="images/{hero_img}" alt="{title}" class="banner-hero-img">
        <div class="banner-sub-bar">
            <div class="container">
                ...
                <h1 class="banner-sub-title">{banner_title}</h1>
                <p>
                    {banner_desc}
                </p>
            </div>
...
"""
    ...
    # Replace the banner section in header
    page_header = re.sub(r'<!-- Panoramic Banner Hero.*?<main id="mainContent">\s*', hero_section, page_header, flags=re.DOTALL)
...
"""
    ...
    full_html = page_header + page_content + commitment_section + footer
...
create_page("psychiatric-evaluations.html", "Psychiatric Evaluations", "Psychiatric Evaluations", "Comprehensive evaluations to understand mental health concerns, history, functioning, and treatment needs.", "service_mental.jpg", content_evaluations)
create_page("medication-management.html", "Medication Management", "Medication Management", "Thoughtful psychiatric prescribing, monitoring effectiveness, education, and ongoing follow-up care.", "service_substance.jpg", content_medication)
create_page("mental-health-assessments.html", "Mental Health Assessments", "Mental Health Assessments", "Identifying emotional, behavioral, and psychiatric concerns affecting relationships, work, and daily life.", "conditions_center.jpg", content_assessments)
create_page("treatment-planning.html", "Treatment Planning", "Treatment Planning", "Custom plans structured around presenting concerns, strengths, and goals, modified as you progress.", "about_doctor_premium.jpg", content_planning)
create_page("education-support.html", "Education & Support", "Patient Education & Support", "Empowering clients and families with insights into diagnoses, coping strategies, and wellness routines.", "legacy_tree.jpg", content_education)

services_grid = """        <section class="services-section section-padding bg-white" id="services-overview">
            <div class="container">
...
services_html = header + services_grid + commitment_section + footer
